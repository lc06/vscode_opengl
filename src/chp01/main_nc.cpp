...
float yaw = -90.0f; // yaw表示偏航角，初始设为0时，表示朝向x轴正向，设为-90.0则指向z轴负向，即朝向物体
float pitch = 0.0f;
...
int main()
{
    ...
    // ------------------------------------
    Shader ourShader("D:/{Code}/Code Repository/vscode_c++/openglcmake/resources/chp01/texture.vs",
                     "D:/{Code}/Code Repository/vscode_c++/openglcmake/resources/chp01/texture.fs");
...
void mouse_callback(GLFWwindow *window, double xpos, double ypos)
{
    ...
    float yoffset = lastY - ypos; // xpos由窗口自左往右递增，ypos由窗口自上往下递增，与OpenGL坐标系的Y坐标方向相反
    lastX = xpos;
    lastY = ypos;
